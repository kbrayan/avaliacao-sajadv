...
export class PessoaService {
  ...
  countPessoasFiltradas(nome: string, email: string, cpf: string): Observable<number> {
    let parametros = new HttpParams();
    parametros = parametros.append('nome', nome);
    parametros = parametros.append('email', email);
    parametros = parametros.append('cpf', cpf);

    return this.http.get<number>(this.countPessoasFiltradasUrl, {params: parametros });
  }

  getPessoasFiltradas(pagina:number, quantidade:number, nome: string, email: string, cpf: string): Observable<Pessoa[]> {
    let parametros = new HttpParams();
    ...
    return this.http.get<Pessoa[]>(this.getPessoasFiltradasUrl, {params: parametros });
  }
  ...
  editPessoa(pessoa: Pessoa): Observable<Pessoa> {
    ...
    console.log(pessoa);
    return this.http.put<Pessoa>(this.pessoaUrl + "/" + pessoa.id, pessoa, httpOptions)
      .pipe(
        retry(1),
        catchError(this.handleError)
      )
